...
class TestPipeline:
    # Verifies that both ETL stages are invoked in a single pipeline run, which is the
    # core contract of the orchestrator and ensures neither stage is accidentally skipped.
    ...
    # The returned dict must contain "silver" and "gold" keys mapping table names to row
    # counts; this structure is what monitoring callers parse to assess pipeline health.
